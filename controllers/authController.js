...
module.exports.forgotPass = async (req, res) => {
  ...
  try {
    ...
    await connection.release();
    // console.log(oldUser[0].idUsuari);

    if (!oldUser.length) return res.status(404).json({ message: "User not found!" });

    const tokenUser = jwt.sign({ email: oldUser[0].mail, id: oldUser[0].idUsuari }, process.env.SECRET_KEY, {
      expiresIn: "5m",
    });

    const link = `http://localhost:3000/reset-password/${oldUser[0].idUsuari}/${tokenUser}`;
